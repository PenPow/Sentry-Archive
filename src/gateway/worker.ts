...
import {
  ...
  GatewayManager,
  GetGatewayBot,
} from "https://deno.land/x/discordeno@13.0.0-rc31/mod.ts";
...
function spawnGateway(shardId: number, options: Partial<GatewayManager>) {
  const prefix = `GatewayWorker ${workerId}`;

  log(
    { level: LogLevel.Info, prefix },
  ...
  gateway = createGatewayManager({
    ...
    handleDiscordPayload: async function (_, data, shardId) {
      ...
      if (data.t === "READY") {
        log({ level: LogLevel.Info, prefix }, `Shard online`);
      ...
      await fetch(`http://${EVENT_HANDLER_URL}:${EVENT_HANDLER_PORT}`, {
        ...
        .catch((err) =>
          log({ level: LogLevel.Error, prefix }, `Error Sending Event:\n${err}`)
        );
    },
  });
...
self.onmessage = async function (message: MessageEvent<string>) {
  ...
  if (data.type === "RESHARDED-CLOSEOLD") {
    log({ level: LogLevel.Info, prefix }, "[Resharding] Closing old gateways.");
    await gateway.resharding.closeOldShards(gatewayPendingClosing);
  }

  if (data.type === "RESHARD") {
    log({ level: LogLevel.Info, prefix }, "[Worker] Resharding the worker.");
    ...
    async function processResharding(
      ...
      for (const [key, _value] of Object.entries(oldGateway)) {
    ...
    log({ level: LogLevel.Info, prefix }, `Resharded the worker.`);
    // @ts-ignore this should not be erroring
    postMessage(
      JSON.stringify({
        type: "RESHARDED",
