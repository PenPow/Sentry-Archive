...
const UnbanCommand: IFunction = {
	...
	async execute(interaction) {
		...
		if (!(await PunishmentManager.canPunish(interaction.client, PunishmentType.Unban, interaction.options.getUser(translate(Locale.EnglishGB, "MODERATION_TARGET_OPTION_NAME"), true).id, interaction.user.id, interaction.guildId))) {
			const embed = new EmbedBuilder()
				.setAuthor({ iconURL: interaction.user.displayAvatarURL(), name: `${interaction.user.tag} (${interaction.user.id})` })
				.setTimestamp()
				.setColor(0xFF5C5C)
				.setTitle(translate(interaction.locale, "CANNOT_UNBAN_USER"));
	...
	toJSON() {
		return {
			...
			options: [{
				...
				description: "UNBAN_MEMBER_OPTION_DESCRIPTION",
