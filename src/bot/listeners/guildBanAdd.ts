...
const guildBanAddEvent: IListener = {
	execute: function(client) {
		client.on("guildBanAdd", async (ban: GuildBan) => {
			...
			const auditLog = (await ban.guild.fetchAuditLogs({ limit: 1, type: AuditLogEvent.MemberBanAdd }).catch(e => void Sentry.captureException(e)))?.entries.first();
			...
			const embed = new EmbedBuilder()
				...
				.setDescription(translate(ban.guild.preferredLocale, "DISCORD_MODERATION_DESCRIPTION", ban, PunishmentType.Ban, translate(ban.guild.preferredLocale, "MODERATION_DEFAULT_REASON")))
