...
const messageCreateEvent: IListener = {
	execute: function(client) {
		client.on("messageCreate", async message => {
			...
			if (['Hypesquad Academy', 'Hypesquad', 'Events', 'Moderator Academy', "Discord", "Moderator"].some(item => similarity(digestedMessage, digest(item.toLowerCase())) > 75)) { total += 150; reason.heatAdded < 150 ? reason = { msg: "Suspicious Username", heatAdded: 150 } : void 0; }
			if ([
			...
			if (/(https?:\/\/)?(www.)?(discord.(gg|io|me|li)|discordapp.com\/invite)\/[^\s/]+?(?=\b)/.exec(message.content)) { total += 80; reason.heatAdded < 80 ? reason = { msg: "Invite Detected", heatAdded: 80 } : void 0; }
