...
const messageCreateEvent: IListener = {
	execute: function(client) {
		client.on("messageCreate", async message => {
			...
			if (settings.username && ['Hypesquad Academy', 'Hypesquad', 'Events', 'Moderator Academy', "Discord", "Moderator"].some(item => similarity(digestedMessage, digest(item.toLowerCase())) > 75)) { total += 150; reason.heatAdded < 150 ? reason = { msg: "Suspicious Username", heatAdded: 150 } : void 0; }
			if (settings.phrase && [
			...
			if (settings.invite && /(https?:\/\/)?(www.)?(discord.(gg|io|me|li)|discordapp.com\/invite)\/[^\s/]+?(?=\b)/.exec(message.content)) { total += 100; reason.heatAdded < 100 ? reason = { msg: "Invite Detected", heatAdded: 100 } : void 0; }

			total += (message.mentions.everyone ? 65 : 0) + (message.mentions.users.size * 15);

			if (message.mentions.everyone || message.mentions.users.size) reason.heatAdded < (message.mentions.everyone ? 300 : message.mentions.users.size * 15) ? reason = { msg: "Mass Mentions", heatAdded: (message.mentions.everyone ? 300 : 0) + (message.mentions.users.size * 15) } : void 0;
				...
				if (res && res.status === 200) {
					...
					if (!raw.success) { log({ level: LogLevel.Error, prefix: 'ClamAV Handler' }, raw.data.error); }
						...
						for (const result of raw.data.result) {
							if (result.is_infected) { total += 500; reason.heatAdded < 500 ? reason = { msg: "Malicious File Upload", heatAdded: 500 } : void 0; }
						}
					}
				}
			}
