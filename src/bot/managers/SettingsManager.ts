...
export const SettingsManager = {
	getSettings: async function(guildId: Snowflake): Promise<Settings> {
		...
		return (await prisma.guild.findUnique({ where: { id: guildId }, include: { settings: true } }))?.settings ?? { id: guildId, automod: true, phrase: true, username: true, invite: false, url: true, spam: true, clamav: true, enforce2fa: false, statistics: true, logChannelId: null, language: "en-GB" };
	},
