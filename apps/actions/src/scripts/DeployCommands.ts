...
export default {
  name: "DeployCommands",
  execute: async () => {
    const Commands: Map<string, { data: RESTPostAPIChatInputApplicationCommandsJSONBody | RESTPostAPIContextMenuApplicationCommandsJSONBody, toJSON(): RESTPostAPIChatInputApplicationCommandsJSONBody | RESTPostAPIContextMenuApplicationCommandsJSONBody } > = await Broker.call('getCommands', null);
    ...
    const [table, registerToGuild] = await io.group([
      io.select.table("Commands", {
        data: [...Commands.values()].map((data, index) => {
          return { id: index, name: data.data.name };
        }),
      }),
    ...
    if (registerToGuild) {
      ...
      await REST.put(
        ...
        { body: selectedCommands.map((command) => command.toJSON()) }
      );
    }
