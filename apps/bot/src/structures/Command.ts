...
export async function loadCommands() {
	...
	for(const action of globbed) {
		...
		const commandExport: Handler<ApplicationCommandType> = (await import(`../${action.replace('dist', '')}`)).default
...
T extends ApplicationCommandOptionType.Channel  ? APIChannel | APIInteractionDataResolvedChannel :
