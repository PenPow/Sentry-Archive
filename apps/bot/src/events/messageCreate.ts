  ...
  async run({ data, logger, api }) {
    ...
    if (settings.enableAV && data.attachments.length > 0) {
      ...
      for (const attachment of data.attachments) {
        ...
        const call: Promise<IClamAVResponse> = AVBroker.call(
          "scan",
          { id: attachment.id, data: buffer },
          1_000_000
        );
        results.push(call);
      }
      ...
      for (const res of resolved) {
        ...
        if (res.data.infected) {
          logger.debug(
            `Attachment ${res.id} is infected with ${res.data.viruses.join(
              " and "
            )}`
          );
    ...
    for (const match of matches) {
      ...
      scans.push(
        fetch("http://phishertools:3001/scan", {
          method: "POST",
          body: JSON.stringify({ domain: url.host }),
        })
      );
    }
