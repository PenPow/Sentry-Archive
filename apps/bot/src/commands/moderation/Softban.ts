...
import { GenericPunishment, NotInGuildPunishment } from "../../structures/Punishment.js";
...
export default class SoftbanCommand extends SlashCommand.Handler<ApplicationCommandType.ChatInput> {
	public override data = {
		name: 'softban',
		description: 'Remove a user from your server and unban them to purge their messages',
	...
	public override options = {
		"user": {
			...
			description: "Which user should Sentry softban?",
			required: true
		},
		"reason": {
	...
	public override async execute({ interaction, respond, getArgs, api }: SlashCommand.RunContext<SoftbanCommand>): SlashCommand.Returnable {
	...
	public override async handleModal({ interaction, respond, api }: Omit<SlashCommand.RunContext<any>, "getArgs" | "interaction"> & { interaction: APIModalSubmitInteraction; }): Promise<void> {
		const codeOption = interaction.data.components[0]?.components.find((input) => input.custom_id.split('.')[5] === "code"); // softban.twofactor.ask.[nanoid].option.[option name]
	...
	public async createPunishment({ interaction, api, options, respond }: { api: API, interaction: APIChatInputApplicationCommandInteraction | APIModalSubmitInteraction, options: Options, respond(interaction: APIChatInputApplicationCommandInteraction | APIModalSubmitInteraction, responseType: ValidDataTypes<APIChatInputApplicationCommandInteraction | APIModalSubmitInteraction>, data: DataType<ValidDataTypes<APIChatInputApplicationCommandInteraction | APIModalSubmitInteraction>>): Promise<Result<true, Error>> }) {
		...
		if(member.isErr()) {
			const punishment = await new NotInGuildPunishment({ userId: user.id, reason, references, guildId: guild.id, moderatorId: interaction.member!.user.id, type: "Softban" }).build();
		...
		const punishment = await new GenericPunishment({ guildId: guild.id, moderatorId: interaction.member!.user.id, references, userId: member.unwrap().user!.id, reason, type: "Softban" }).build();
