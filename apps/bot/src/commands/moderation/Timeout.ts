...
export default class TimeoutCommand extends SlashCommand.Handler<ApplicationCommandType.ChatInput> {
	...
	public override async execute({ interaction, respond, getArgs, api }: SlashCommand.RunContext<TimeoutCommand>): SlashCommand.Returnable {
		...
		if(userHas2FA) {
			...
			inProgress.set(id, { user: getArgs(interaction, "user"), reason: getArgs(interaction, "reason"), expiration: getArgs(interaction, "duration") ?? undefined, references: getArgs(interaction, "case_reference") ?? undefined });
		} else {
			await respond(interaction, CommandResponseType.Defer, { flags: MessageFlags.Ephemeral });
			await this.createPunishment({ interaction, api, options: { reason: getArgs(interaction, "reason"), user: getArgs(interaction, "user"), expiration: getArgs(interaction, "duration") ?? undefined, references: getArgs(interaction, "case_reference") ?? undefined}, respond });
		}
	}
