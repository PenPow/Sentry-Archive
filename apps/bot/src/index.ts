					...
					if("options" in interaction.data) {
						...
						if([ApplicationCommandOptionType.String, ApplicationCommandOptionType.Number, ApplicationCommandOptionType.Integer, ApplicationCommandOptionType.Boolean].includes(option.type)) return option.value;
							...
							if(option.type === ApplicationCommandOptionType.Channel) {
								const fetched = (interaction.data.resolved?.channels && interaction.data.resolved?.channels[option.value]) ?? await REST.get(Routes.channel(option.value)) as Exclude<APIChannel, APIGroupDMChannel | APIDMChannel>;
