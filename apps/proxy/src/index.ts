...
function proxyRequests(rest: REST): (req: IncomingMessage, res: ServerResponse) => Promise<void> | void {
	return async (req, res) => {
		...
		const route = parsedUrl.pathname.replace(/^\/api(\/v\d+)?/, '') as RouteLike;
...
if(!config.discord.TOKEN || ![6, 7, 8, 9, 10].includes(config.proxy.API_VERSION)) {
	throw new Error("Invalid Config");
}

const rest = new REST({ rejectOnRateLimit: () => true, retries: 0, version: config.proxy.API_VERSION.toString() }).setToken(config.discord.TOKEN);
const server = createServer(proxyRequests(rest));

server.listen(config.proxy.PORT, () => logger.info(`Listening on http://localhost:${config.proxy.PORT}`));