...
LABEL org.opencontainers.image.description="Sentry Phishing Database Image"
...
RUN npx turbo run build --filter=phishertools
...
COPY --from=installer /app/apps/phishertools/package.json .
COPY --from=installer /app/node_modules ./node_modules
COPY --from=installer --chown=sentry:nodejs /app/apps/phishertools/dist ./dist
